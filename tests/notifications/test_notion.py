...
class TestPropertyMapping:
    ...
    def test_notes_field_stamped_with_auto_marker(self):
        # Every agent-created row carries a marker in 備註 so the user
        # can distinguish agent-created rows from manually-added ones.
        recorder = _recorder_with_mock_client()
        recorder.record_expense(_record())
        ...
        assert props["備註"] == {
            "rich_text": [
                {"text": {"content": "auto generated by zashiki-warasi"}}
            ],
        }

    def test_notes_marker_present_even_when_all_optional_fields_none(self):
        # 備註 is independent of LLM extraction — it's always there.
        recorder = _recorder_with_mock_client()
        recorder.record_expense(
            _record(
                title=None,
        ...
        props = recorder._client.pages.create.call_args.kwargs["properties"]
        assert (
            props["備註"]["rich_text"][0]["text"]["content"]
            == "auto generated by zashiki-warasi"
        )
