...
Grafana Prometheus datasource so the bundled starter dashboard's
`"uid": "prometheus"` references resolve on a fresh install — no
operator UI intervention needed. Known-issue since v1.1 (documented
but not patched at the config layer until now). K3s side unaffected.
