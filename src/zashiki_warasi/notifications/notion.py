...
class NotionExpenseRecorder:
    ...
    PROP_NOTES = "備註"  # rich_text — stamped with AUTO_GENERATED_NOTE

    # Marker written into every row's 備註 column so the user can tell
    # at a glance which Notion rows the agent created vs. ones they
    # added manually.
    AUTO_GENERATED_NOTE = "auto generated by zashiki-warasi"
    ...
    def _build_properties(self, record: ExpenseRecord) -> dict[str, Any]:
        ...
        # Always stamp 備註 with the agent marker so manually-added
        # rows stay distinguishable.
        properties[self.PROP_NOTES] = {
            "rich_text": [{"text": {"content": self.AUTO_GENERATED_NOTE}}],
        }
