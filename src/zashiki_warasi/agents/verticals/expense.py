...
10. 條款 vs. 實際交易(重要):
    ...
    不要拿它們填 amount / vendor / transaction_id 等欄位。
    - 手續費說明(例如「海外ATMでの引き出しは...ATM利用手数料
      110円を加えて引き落とし致します」、「送金手数料 無料」)。
    - 假設情境條款(例如「為替影響で変更があった場合...」)。
    - 客服/活動/注意事項連結與說明。
    判斷:實際交易一定同時有具體的日期時間 + 店家 + 金額。
    boilerplate 通常只有金額而缺日期或店家 — 這種數字**不是**
    這筆消費的金額。
    典型案例:SMBC Olive デビット的「ご利用のお知らせ」— 上方
    「◇利用日 / ◇利用先 / ◇利用金額 / ◇承認番号」是真實交易,
    下方「海外ATM 手数料 110 円」是條款,只取上方一筆。
