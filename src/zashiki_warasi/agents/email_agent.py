...
- 「消費資訊彙整」的判斷關鍵:郵件裡有 **多筆** 都具備日期時間 +
  店家 + 金額三者齊全的實際交易。若只有 **一筆** 真實交易,
  即便下方含有手續費條款、海外 ATM 說明、為替影響提醒等
  boilerplate,仍歸為「消費支出」— boilerplate 條款不算獨立交易。
  典型例:SMBC Olive デビット的「ご利用のお知らせ」通常是單筆
  消費通知(承認番号是 transaction_id),即便信件下方寫著
  「海外ATMでの現地通貨の引き出しは...ATM利用手数料110円を
  加えて引き落とし致します」這種條款,仍為「消費支出」,不要
  把 110 円當成第二筆消費。
